...
function backStep(){ if(E.round>0) E.addBtn(t({en:'◀ Prev step',zh:'◀ 上一步'}),'',E.prevStep); }   // replay an earlier step (no EXP — already cleared)
...
function bg(){ const ctx=E.ctx,LW=E.LW,LH=E.LH; E.clear(); FIG.fog(ctx,0,LW,0,LH*0.55,performance.now());
  ctx.fillStyle='#15301f'; ctx.fillRect(0,LH-50,LW,50); ctx.fillStyle='#2f7a3f'; ctx.fillRect(0,LH-54,LW,5); FIG.bull(ctx,40,LH-80,46); }
...
try{ ['happy','sad'].forEach(tauImg); }catch(_){}   // warm the cache so the emblem is decoded before the first graze
function calf(ctx,x,y,r,fed){ const img=tauImg(fed?'happy':'sad'), s=r*3.2;
  if(img.complete && img.naturalWidth){ ctx.save(); if(!fed)ctx.globalAlpha=.5; try{ ctx.drawImage(img,x-s/2,y-s/2,s,s); }catch(_){ } ctx.restore(); }
  else { ctx.save(); ctx.fillStyle=fed?'#f4c830':'rgba(126,122,104,.6)'; ctx.beginPath(); ctx.arc(x,y,r,0,7); ctx.fill(); ctx.restore(); } }   // fallback dot until the emblem decodes
...
function waitingHerd(n){ if(n<=0)return; const ctx=E.ctx,LW=E.LW,LH=E.LH; const sp=Math.min(28,(LW-170)/Math.max(n-1,1)), x0=Math.max(100,(LW-(n-1)*sp)/2), r=Math.min(11,sp*0.42);
  for(let i=0;i<n;i++) calf(ctx,x0+i*sp,LH-24,r,true); }
...
function gfield(b,c,a,g,w,copies,opt){ opt=opt||{}; bg(); const ctx=E.ctx,LW=E.LW,LH=E.LH;
  ...
  const centers=[];
  for(let r=0;r<copies;r++)for(let k=0;k<g+w;k++){ const x=ox+k*cell,y=oy+r*cell;
  ...
  for(let r=0;r<a;r++)for(let k=0;k<bc;k++){ if(!(r<copies&&k<g+w)) outl(ctx,ox+k*cell,oy+r*cell,cell,k<b?GREEN:GOLD); }
  wall(ctx, ox+bc*cell, oy-3, gh+6);                  // outer wall — the field's width; tiles past it fall on rock
  if(b>0&&c>0) wall(ctx, ox+b*cell, oy-3, a*cell+6);  // grass ends here — move on to wheat
  label(ox+bc*cell/2,oy-12, (b>0&&c>0)? g+'+'+w : (c>0? ''+w : ''+g), FIG.C.text);
  vlabel(ref? ox+bc*cell+16 : ox-14, oy+(Math.max(copies,1)*cell)/2, copies);   // when a reference '=' sits on the left, put the row-count on the field's right edge
  return {cell,centers}; }
...
function runRound(E, cfg){ E.setSpeaker('tau'); E.mood('idle'); E.setDots(cfg.dot); E.cv.onclick=null;
  ...
  function refresh(){ gfield(B,C,A,g,w,k,{ref:cfg.ref}); waitingHerd(N); magician(E.ctx,32,E.LH*0.42,46,0); label(32,E.LH*0.42+34,t({en:'Product',zh:'“乘”'}),'#caa84a',12);
    ...
    if(k===1){   // copies=1: still shaping the group — the row is editable
      if(B>0){ E.addBtn(t({en:'grass +',zh:'青草 +'}),'',()=>{ if(g<(C>0?B:B+3)){g++;E.sfx('place');refresh();} else E.pop(C>0?t({en:'grass wall — use wheat',zh:'青草到墙了——用麦子'}):t({en:'over the wall!',zh:'越墙了！'})); });
        E.addBtn(t({en:'grass −',zh:'青草 −'}),'',()=>{ if(g>0){g--;E.sfx('tick');refresh();} }); }
      if(C>0){ E.addBtn(t({en:'wheat +',zh:'麦子 +'}),'',()=>{ if(w<C+3){w++;E.sfx('place');refresh();} else E.pop(t({en:'plenty!',zh:'够了！'})); });
        E.addBtn(t({en:'wheat −',zh:'麦子 −'}),'',()=>{ if(w>0){w--;E.sfx('tick');refresh();} }); }
      E.addBtn(t({en:'✦ Product: copy the row ✦',zh:'✦ “乘”：复制此行 ✦'}),'primary',()=>{ k++; cast(); });
    } else {     // the row is locked as the group — now you multiply it
      E.addBtn(t({en:'✦ copy again ✦',zh:'✦ 再复制一份 ✦'}),'',()=>{ if(k<A+3){k++; cast();} else E.pop(t({en:'plenty!',zh:'够了！'})); });
      E.addBtn(t({en:'− a copy',zh:'− 一份'}),'',()=>{ k--; E.sfx('tick'); refresh(); });
      E.addBtn(t({en:'↺ reshape the row',zh:'↺ 重排此行'}),'',()=>{ k=1; E.sfx('tick'); refresh(); });
    }
    E.addBtn(t({en:'Send the herd ▶',zh:'放牛 ▶'}),'primary',send); }
  function cast(){ E.busy=true; E.sfx('bracket'); E.pop('✦ copy! ✦');
    E.anim(480, p=>{ const geo=gfield(B,C,A,g,w,k,{ref:cfg.ref}); waitingHerd(N); magician(E.ctx,32,E.LH*0.42,48,p);
  ...
  function send(){ if(E.busy)return; const inRow=Math.min(g+w,bc), graze=k*inRow, rockN=k*Math.max(0,(g+w)-bc), draw=grow=>gfield(B,C,A,g,w,k,{grow:grow,ref:cfg.ref});
    ...
    const msg = rockN>0 ? t({en:'<b class="r">'+rockN+'</b> tiles spilled onto bare <b class="r">rock</b> past the wall — keep the row <b>'+(C>0?B+'+'+C:''+B)+'</b> wide.',zh:'<b class="r">'+rockN+'</b> 格落到墙外的光<b class="r">石头</b>上——把行保持在 <b>'+(C>0?B+'+'+C:''+B)+'</b> 宽。'})
...
function round1(E){ const pr=[[2,3],[2,4],[3,4],[2,5]][rnd(0,3)]; const a=pr[0], b=pr[1], N=a*b; baseline={a,b,N};
  runRound(E,{ dot:0, B:b, C:0, A:a, N:N,
    ...
    intro:t({en:'<b>The Walled Meadow.</b> <b>'+N+'</b> calves wait below. Build <b>one row</b> of <b class="g">'+b+'</b> grass, then let <b>Product</b> the magician <b>copy</b> it — enough times to feed them all.',zh:'<b>围栏草甸。</b><b>'+N+'</b> 头小牛在下面等着。先铺<b>一行</b> <b class="g">'+b+'</b> 格青草，再让魔法师<b>“乘”</b>把它<b>复制</b>——复制到够喂饱所有牛。'}),
    win:()=>{ E.setDots(1); E.tickQ(1); E.award(45); E.status(keq(a+' × '+b+' = '+N));
      ...
      E.clearTray(); E.addBtn(t({en:'On to the Narrow Strip ▶',zh:'前往窄田 ▶'}),'primary',E.advance); E.addBtn(t({en:'↻ Replay (no EXP)',zh:'↻ 重玩（无经验）'}),'',E.replayStep); } });
}
...
function round2(E){ const a=baseline.a, b=baseline.b, N=baseline.N;
  runRound(E,{ dot:1, B:a, C:0, A:b, N:N,
    ...
    intro:t({en:'<b>The Narrow Strip.</b> The same <b>'+N+'</b> calves — but this strip pens each row to just <b class="g">'+a+'</b> wide (the meadow\'s row was '+b+'). Build the <b>'+a+'</b>-row, then let Product copy it.',zh:'<b>窄田。</b>还是这 <b>'+N+'</b> 头牛——但这片窄田把每行限到只有 <b class="g">'+a+'</b> 宽（草甸那行是 '+b+'）。铺好这 <b>'+a+'</b> 行，再让“乘”复制。'}),
    win:()=>{ E.setDots(2); E.tickQ(2); E.award(50);
      ...
      E.tell(t({en:'See it: the faded meadow <b>'+a+'×'+b+'</b> holds the same <b>'+N+'</b> as your strip <b>'+b+'×'+a+'</b>. Flip the group, the product holds — <b>'+a+'×'+b+' = '+b+'×'+a+'</b>. That\'s <b>commuting</b>.',zh:'看：淡色的草甸 <b>'+a+'×'+b+'</b> 和你的窄田 <b>'+b+'×'+a+'</b> 装着同样的 <b>'+N+'</b>。把组调换，乘积不变——<b>'+a+'×'+b+' = '+b+'×'+a+'</b>。这就是<b>交换律</b>。'}));
      E.clearTray(); E.addBtn(t({en:'On to the Shared Fence ▶',zh:'前往共栏 ▶'}),'primary',E.advance); E.addBtn(t({en:'↻ Replay (no EXP)',zh:'↻ 重玩（无经验）'}),'',E.replayStep); } });
}
...
function round3(E){ const a=rnd(2,3), b=rnd(2,3), c=rnd(2,3); const T=a*(b+c), gP=a*b, wP=a*c;
  const place=t({en:'The Shared Fence',zh:'共栏'});
  function grassPhase(){ runRound(E,{ dot:2, B:b, C:0, A:a, N:gP, place:place,
    intro:t({en:'<b>The Shared Fence — Part 1.</b> Count each crop on its own. First the <b class="g">grass</b>: build a row of <b class="g">'+b+'</b>, then Product copies it <b>'+a+'</b> times → <b class="g">'+a+'×'+b+'</b>.',zh:'<b>共栏——第一部分。</b>把每种作物分开数。先种<b class="g">青草</b>：铺一行 <b class="g">'+b+'</b>，再让“乘”复制 <b>'+a+'</b> 次 → <b class="g">'+a+'×'+b+'</b>。'}),
    win:()=>{ E.status(keq(a+' × '+b+' = '+gP)); E.tell(t({en:'<b class="g">Grass</b> done: <b class="g">'+a+'×'+b+' = '+gP+'</b>. Now grow the <b class="o">wheat</b> on its own.',zh:'<b class="g">青草</b>种好：<b class="g">'+a+'×'+b+' = '+gP+'</b>。现在单独种<b class="o">麦子</b>。'})); E.clearTray(); E.addBtn(t({en:'Now the wheat ▶',zh:'再种麦子 ▶'}),'primary',wheatPhase); E.addBtn(t({en:'↻ Replay (no EXP)',zh:'↻ 重玩（无经验）'}),'',E.replayStep); } }); }
  function wheatPhase(){ runRound(E,{ dot:2, B:0, C:c, A:a, N:wP, place:place, ref:{r:a,c:b,op:'+'},
    intro:t({en:'Now the <b class="o">wheat</b>, separately (your <b class="g">'+a+'×'+b+'</b> grass stays on the left): build a row of <b class="o">'+c+'</b>, then Product copies it <b>'+a+'</b> times → <b class="o">'+a+'×'+c+'</b>.',zh:'现在单独种<b class="o">麦子</b>：铺一行 <b class="o">'+c+'</b>，再让“乘”复制 <b>'+a+'</b> 次 → <b class="o">'+a+'×'+c+'</b>。'}),
...
const QUEST = {
  ...
  meta:{ title:{en:'Sow the Pasture',zh:'播种牧场'}, giver:{en:'Tau the Calf · The Cradle',zh:'小牛 Tau · 摇篮'},
    flavor:{en:'"We crossed the bridges — thank you! But a few calves still isn\'t a herd. Help me grow a <b>pasture</b>. The land here is cut by <b>cliffs</b> into walled plots — so a field of grass is <b>rows × columns</b>, an <b>area</b>. Plant it well and the whole herd grows strong. Let\'s sow!"',
      zh:'"桥我们过来啦——谢谢你！可几头小牛还成不了牛群。帮我种一片 <b>牧场</b>吧。这里的土地被<b>峭壁</b>切成一块块围栏田——所以一片草地就是 <b>行 × 列</b>，也就是<b>面积</b>。种好了，整群牛都会强壮起来。开种吧！"'} },
  objs:[ {en:'The Walled Meadow: area = rows × columns',zh:'围栏草甸：面积 = 行 × 列'},
         {en:'The Narrow Strip: turning is free',zh:'窄田：转动自由'},
         {en:'The Shared Fence: split a field freely',zh:'共栏：田可自由拆分'} ],
  rounds:[round1,round2,round3],
  book:{ page:2, kicker:{en:'Introduction',zh:'入门之'}, title:{en:'Multiplication',zh:'乘法'},
    blocks:[
      {top:true, fig:'mjumps', prose:{en:'On the number line, addition was a <b>jump</b>. Take the <b class="q">same</b> jump again and again — <b>2 + 2 + 2</b> — and that is <b>3 × 2</b>. Multiplication is just repeated addition.',zh:'在数轴上，加法是一<b>跳</b>。把<b class="q">同样</b>的一跳重复几次——<b>2 + 2 + 2</b>——就是 <b>3 × 2</b>。乘法不过是重复相加。'}},
      {top:true, fig:'marr', prose:{en:'Line those <b class="q">equal</b> groups up as rows, and the product becomes an <b class="len">area</b> — a rectangle of tiles.',zh:'把这些<b class="q">相同</b>的组排成一行行，乘积就成了<b class="len">面积</b>——一块由格子拼成的长方形。'}},
      {top:true, fig:'mrect', prose:{en:'A field of <b>'+'a'+'</b> rows and <b>b</b> columns holds <b class="len">a × b</b> tiles — its area.',zh:'<b>a</b> 行 <b>b</b> 列的田，有 <b class="len">a × b</b> 格——这就是它的面积。'}},
      {law:{en:'Commutative',zh:'交换律'}, eq:'2 × 3 = 3 × 2', fig:'mrot', prose:{en:'Turn the rectangle a quarter — same area.',zh:'把长方形转四分之一圈——面积不变。'}},
      {law:{en:'Associative',zh:'组合律'}, eq:'(2 × 3) × 4 = 2 × (3 × 4)', fig:'mgroups', prose:{en:'Multiply three numbers grouping them any way — the product is the same.',zh:'三个数相乘，无论先乘哪两个，乘积都一样。'}},
      {law:{en:'Distributive',zh:'分配律'}, eq:'a×(b+c) = a×b + a×c', fig:'mdist', prose:{en:'Glue two rectangles that share a side, and you can pull the shared side out.',zh:'把共用一条边的两个长方形拼起来，就能把那条公共边提出来。'}},
      {note:{en:'<b>Aside.</b> Length built segments; now area builds fields — and soon, curved shapes.',zh:'<b>进阶。</b> 长度搭出了线段，面积铺出了田地——接下来，是弯曲的形状。'}}
    ],
