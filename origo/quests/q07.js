...
function round1(E){ E.setSpeaker('tau'); E.mood('idle'); E.setDots(0); E.sceneStop(); PIST='loom'; pmood='idle'; pSad=0;
  ...
  function radiusCut(n){ const ctx=E.ctx, ox=CX(), oy=CY(), R0=R(), dx=R0/n;
    ...
    const cnt=(n>=NMAX)?'N':n; label(ox, oy-R0-16, t({en:'radius is cut into '+cnt+' pieces',zh:'半径被切成 '+cnt+' 份'}), GOLD, 14, true);   // live caption — the count becomes the symbol N at the far right
    label(ox, oy+R0+18, t({en:'dx = one step — “delta-x”, a tiny change in x (one symbol, not d × x)',zh:'dx = 一步——「delta-x」，x 的微小变化（一个整体符号，不是 d × x）'}), '#ffb39c', 11, true); }   // explain dx on-screen
...
function round2(E){ E.setSpeaker('tau'); E.mood('idle'); E.setDots(1); E.sceneStop(); PIST='loom'; pmood='idle';
  ...
  function dottedBar(){ const ctx=E.ctx; ctx.save(); ctx.strokeStyle='rgba(80,216,144,.4)'; ctx.setLineDash([4,4]); ctx.lineWidth=1.4; ctx.strokeRect(C2x(),barY()-dxw()/2,barLen(),dxw()); ctx.restore(); }
  function drawAll(extra){ bg(); innerRings(); if(p<0.99) dottedBar(); band(); odotL(); if(extra)extra(); }
  const handle={ kind:'drag', home:{x:C2x(),y:barY()}, drag:{axis:'x',clamp:{x0:C2x(),x1:C2x()+barLen(),y0:barY(),y1:barY()}}, bbox:a=>({x:a.pos.x-16,y:a.pos.y-16,w:32,h:32}), hiCol:'rgba(80,216,144,.9)' };
  E.tell(t({en:'<b>Straighten a Ring.</b> The <b>outer</b> <b class="y">ring</b> (rim <b class="y">τ</b>, thickness <b class="r">dx</b>) is cut at the bottom — its RIGHT cut-end already sits at the bar\'s left end. <b>Drag the LEFT cut-end along to the bar\'s right end</b> and the whole ring straightens into a bar.',
    zh:'<b>拉直圆环。</b><b>最外</b>那圈<b class="y">环</b>（圆边 <b class="y">τ</b>，厚 <b class="r">dx</b>）在底部切开——它的右切口已经落在长条的左端。<b>把左切口沿着拖到长条的右端</b>，整圈就拉直成一根长条。'}));
  E.scene({ actors:[handle], draw:()=>{ p=Math.max(0,Math.min(1,(handle.pos.x-C2x())/barLen())); drawAll(()=>{ const ctx=E.ctx, hp=handle.grab?handle.pos:{x:C2x(),y:barY()}; ctx.save(); ctx.fillStyle=RD; ctx.shadowColor='rgba(255,106,77,.7)'; ctx.shadowBlur=8; ctx.beginPath(); ctx.arc(hp.x,hp.y,8,0,7); ctx.fill(); ctx.restore(); if(p<0.08) label(C2x()+barLen()/2,barY()+22,t({en:'drag the cut-end all the way right →',zh:'把切口端一路拖到右端 →'}),'#ffd9b0',11,true); }); },
    onDrop(a,z,info){ if(E.busy)return; if(p>=0.85){ E.busy=true; E.sceneStop(); E.sfx('place'); E.mood('happy'); pmood='hurt'; E.anim(220,q=>{ p=0.85+0.15*q; drawAll(); }, ()=>{ p=1; E.busy=false; decide(); }); }
      else { E.oops(); E.status('<span style="color:#ff6a4d">'+t({en:'drag it all the way to the bar’s right end.',zh:'一路拖到长条的右端。'})+'</span>'); } } });
  function decide(){ p=1;
    function q1(){ pickPills(t({en:'The bar IS the outer <b class="y">ring</b> unrolled, so its <b>base</b> is the ring\'s whole rim, <b class="y">τ</b>. So <b class="y">b</b> = ?',zh:'这根长条就是最外<b class="y">环</b>摊开的样子，它的<b>底</b>就是整条圆边 <b class="y">τ</b>。所以 <b class="y">b</b> = ?'}),
        ()=>drawAll(()=>label(C2x()+barLen()/2,barY()-dxw()/2-12,'b',GOLD,15,true)), E.LH*0.92,
        [ {txt:{en:'b = τ',zh:'b = τ'}, ok:true},
          {txt:{en:'b = dx',zh:'b = dx'}, fb:{en:'dx is the thin THICKNESS. The long base is the unrolled rim, τ.',zh:'dx 是薄薄的厚度。长底是摊开的圆边，τ。'}},
          {txt:{en:'b = 1',zh:'b = 1'}, fb:{en:'1 is the radius. The outer ring\'s rim is τ.',zh:'1 是半径。最外环的圆边是 τ。'}} ], q2); }
    function q2(){ pickPills(t({en:'And the bar\'s <b>height</b> is the ring\'s thickness — one tiny step. So <b class="r">h</b> = ?',zh:'而长条的<b>高</b>就是环的厚度——一小步。所以 <b class="r">h</b> = ?'}),
        ()=>drawAll(()=>{ label(C2x()+barLen()/2,barY()-dxw()/2-12,'b = τ',GOLD,14,true); label(C2x()-12,barY(),'h',RD,14,true); }), E.LH*0.92,
        [ {txt:{en:'h = dx',zh:'h = dx'}, ok:true},
          {txt:{en:'h = τ',zh:'h = τ'}, fb:{en:'τ is the long base. The height is the thin thickness — dx.',zh:'τ 是长底。高是薄薄的厚度——dx。'}},
          {txt:{en:'h = 1',zh:'h = 1'}, fb:{en:'1 is the radius. The ring is one step thick: h = dx.',zh:'1 是半径。环只有一步厚：h = dx。'}} ], win); }
    q1();
  }
  function win(){ E.setDots(2); E.tickQ(2); E.award(45); pmood='hurt'; drawAll(()=>{ label(C2x()+barLen()/2,barY()-dxw()/2-12,'b = τ',GOLD,14,true); label(C2x()-12,barY(),'h = dx',RD,11,true); });
    E.cheer(); E.sfx('win');
...
const QUEST = {
  ...
  meta:{ title:{en:'Measure the area — a second way',zh:'再量一次面积'}, giver:{en:'Master Curvo · The Coil',zh:'曲师 Curvo · 盘卷'},
    flavor:{en:'"One proof, <b class="p">Pi</b> sneers, proves nothing: <i>“crumbs measure nothing!”</i> So <b>measure the area a SECOND way</b> and beat him twice, meeting the tool that powers everything still to come. Take a fresh <b>donut</b> from my oven and slice the <b class="r">radius</b> into thin <b class="y">rings</b>, each one tiny step <b class="r">dx</b> wide; your calf <b class="y">Tau</b> tugs a ring loose and unrolls it into a strip, <b class="r">dx</b> tall and its rim long, then stacks them all, short to tall, into a <b class="g">triangle</b> (base the <b class="r">radius</b>, height the rim <b class="y">τ</b>). Its area? <b class="y">½τ</b>, the very same as the pizza cut. Two roads, one truth: <b class="p">Pi</b> is driven off, and the little <b class="r">dx</b> is yours to keep."',
      zh:'"一种证法，<b class="p">Pi</b> 冷笑，算不得数：<i>“碎屑量得出什么！”</i>那就<b>用第二种办法再量一遍面积</b>，把他驳倒两回，顺便认识那件驱动后续一切的工具。从我炉里拿一个新鲜<b>甜甜圈</b>，把<b class="r">半径</b>切成一圈圈细<b class="y">环</b>，每环宽一小步 <b class="r">dx</b>；你的小牛 <b class="y">Tau</b> 把一个环拽松、摊成一条带，高 <b class="r">dx</b>，长是它的圆边，再把它们从短到长摞成一个<b class="g">三角</b>（底是<b class="r">半径</b>，高是圆边 <b class="y">τ</b>）。面积呢？<b class="y">½τ</b>，和披萨切法分毫不差。两条路，同一个真相：<b class="p">Pi</b> 被赶跑，那小小的 <b class="r">dx</b> 就归你了。"'} },
  objs:[ {en:'Cut the Radius: N steps, N·dx = 1',zh:'切开半径：N 步，N·dx = 1'},
         {en:'Straighten the outer ring: base τ, height dx',zh:'拉直最外环：底 τ，高 dx'},
         {en:'Double it: two triangles = τ rectangle → ½τ = π',zh:'翻一倍：两个三角 = τ 矩形 → ½τ = π'} ],
  rounds:[round1,round2,round3],
  book:{ page:7, kicker:{en:'The Coil',zh:'盘卷之'}, title:{en:'Area of the Unit Disk · Donut Rings',zh:'单位圆盘的面积 · 甜甜圈法'},
    blocks:[
      {top:true, fig:'drings', prose:{en:'A second road to the same area. Cut the <b class="r">radius</b> into <b>N</b> tiny steps, each <b class="r">dx</b> (so <b class="r">N·dx = 1</b>), and swing each step into a <b class="y">ring</b>, like tree-rings. This little <b class="r">dx</b> (“delta-x”, one symbol) is the key tool of everything ahead.',
        zh:'通往同一面积的第二条路。把<b class="r">半径</b>切成 <b>N</b> 小步，每步 <b class="r">dx</b>（于是 <b class="r">N·dx = 1</b>），再把每一步绕成一圈<b class="y">环</b>，像年轮。这个小小的 <b class="r">dx</b>（「delta-x」，一个整体符号）是后面一切的关键工具。'}},
      {law:{en:'Straighten a ring → a bar',zh:'拉直一个环 → 一条'}, fig:'dstrip', prose:{en:'A <b class="y">ring</b> is a loop of rim, so straightened it is a thin <b class="g">bar</b>: height <b class="r">dx</b> (its thickness), and as long as its rim. The <b>outer</b> ring\'s rim is the whole turn <b class="y">τ</b>. Stack every ring left-aligned at the cut and they lean into a <b class="g">triangle</b>: base <b class="y">τ</b>, height <b class="r">1</b>.',
        zh:'<b class="y">环</b>是一圈圆边，拉直就是一条细<b class="g">条</b>：高 <b class="r">dx</b>（它的厚度），长就是它的圆边。<b>最外</b>环的圆边是整整一圈 <b class="y">τ</b>。把每个环靠切口左对齐摞起来，它们斜成一个<b class="g">三角</b>：底 <b class="y">τ</b>，高 <b class="r">1</b>。'}},
      {law:{en:'Double it: two triangles = a <span class="y">τ</span> rectangle',zh:'翻一倍：两个三角 = <span class="y">τ</span> 矩形'}, eq:'two <span class="g">△</span> = <span class="y">τ</span> × <span class="r">1</span> = <span class="y">τ</span>  →  each <span class="g">△</span> = <span class="y">½τ</span> = <span class="p">π</span>', fig:'dtri', prose:{en:'We have no triangle-area rule <i>yet</i>, so <b>double</b> it. A second disk\'s triangle, turned upside-down, fits on top to make a <b>rectangle</b>: base <b class="y">τ</b>, height <b class="r">1</b>, area <b class="y">τ</b>. Two equal triangles fill it, so each <b class="g">triangle</b> (a disk) is <b class="y">½τ</b> = <b class="p">π</b>, the <b>same</b> as the pizza cut.',
        zh:'我们<i>还没</i>有三角形面积公式，那就<b>翻一倍</b>。把第二个圆盘的三角倒过来扣上，拼成一个<b>矩形</b>：底 <b class="y">τ</b>，高 <b class="r">1</b>，面积 <b class="y">τ</b>。两个相等的三角拼满它，所以每个<b class="g">三角</b>（一个圆盘）是 <b class="y">½τ</b> = <b class="p">π</b>，和披萨切法<b>一样</b>。'}},
    ...
    read:{en:'Here is a second way to the disk area. Cut the radius into N tiny steps, each dx, so N times dx is 1, and swing each step into a ring. Straighten the rings into bars: the outer ring has rim tau and thickness dx. Stack them left-aligned and they make a triangle, base tau and height 1. We have no triangle-area rule yet, so double it: a second triangle, turned upside-down, makes a rectangle of base tau and height 1, area tau. Two equal triangles, so each is half tau, about 3.14. That is pi: a disk has area half tau. Cutting into tiny dx steps and stacking is exactly what an integral is.',
      zh:'这是求圆盘面积的第二种办法。把半径切成 N 小步，每步 dx，于是 N 乘 dx 等于 1，再把每一步绕成一圈环。把环拉直成条：最外环的圆边是 tau，厚 dx。把它们左对齐摞起来，拼成一个三角，底 tau、高 1。我们还没有三角形面积公式，那就翻一倍：第二个三角倒过来，拼成底 tau、高 1 的矩形，面积 tau。两个相等的三角，所以每个是半个 tau，大约 3.14。那正是 pi：圆盘面积是半个 tau。把图形切成微小的 dx 步再摞起来，这正是积分。'} },
